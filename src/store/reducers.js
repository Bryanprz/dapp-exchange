...
function exchange(state = {}, action) {
  ...
  switch (action.type) {
    ...
    case 'BUY_ORDER_MAKING':
      return { ...state, buyOrder: { ...state.buyOrder, amount: null, price: null, making: true} } // clear amount/price during order processing
    case 'ORDER_MADE':
      // Prevent duplicate orders
      index = state.allOrders.data.findIndex(order => order.id === action.order.id);

      if(index === -1) { 
        // add order if not already in allOrders
        data = [...state.allOrders.data, action.order]
      } else { 
        // don't add order if already in allOrders
